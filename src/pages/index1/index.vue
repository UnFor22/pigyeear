    ...
    <div class="card">
      ...
      <div class="banner">
        ...
        <div class="lamititleswiper" style="width: 100%; height: 30rpx;margin-top:30rpx">
          ...
          <!--  <div id="box">
              <ul id="con1" ref="con1" :class="{anim:animate==true,ma:isActive }">
                <li v-for="(item, index) in bannerDataTxt" :key="index">
                    <a :href=item.url>
                        <span>{{item.title}}</span>
                    </a>
                </li>
      ...
      <div class="hotCard" ref="test">
        ...
        <div class="hotCard_content">
          <!-- <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="bottomAllLoaded" :auto-fill="false" ref="loadmore" :bottomPullText='bottomText' :bottomDropText="bottomDropText" :distance-index="1.2"> -->
            <ul>
              <li v-for="(item, index) in pageList" :key="index" @click="hotdetailsNum(index)">
                ...
                <!--<router-link  :to="{path: '/hotcredit', query: {url: item.crediturl,creditname:item.creditname}}" style="width: 100%; height: 100%" >
                  <img :src=item.creditphotourl alt="">
                  <div>
                    <p>{{item.creditname}}</p>
                    <p>{{item.creditcontent}}</p>
  ...
  export default {
    ...
    methods: {
      ...
      toProgresscheck() {
        wx.navigateTo({ 
          // url: '../workingMode/workingMode?workModeAndPriceList=' + JSON.stringify(workModeAndPriceList) + '&deviceId=' + deviceId,
          url: "/pages/1/main?utm_source=" 
          // + urlList.urlUtm + '&mob' + 'urlList.urlMob' 
        });
      },
      // 跳转到激活页
      toActivation() {
        wx.navigateTo({ 
          // url: '../workingMode/workingMode?workModeAndPriceList=' + JSON.stringify(workModeAndPriceList) + '&deviceId=' + deviceId,
          url: "/pages/index1/main?utm_source=" 
          // + urlList.urlUtm + '&mob' + 'urlList.urlMob' 
        });
      },
      ...
      toNovicecard(){
        wx.navigateTo({ 
          ...
          url: "/pages/index1/main?utm_source=" 
          // + urlList.urlUtm + '&mob' + 'urlList.urlMob' 
         // {path: '/largeamount', query: {utm_source:urlList.urlUtm,mob:urlList.urlMob}
        });
      },
      /*监听页面滚动*/
      // handleScroll(){
      //     this.isScrollIcon=false;
      // },
      
      /*判断滚动停止*/
      // bindtouchend(){
      //      window.urlChannel = window.location.href;
      //     //判断url是否包含type
      //     if(urlChannel.indexOf('type=')>-1){
      //         this.isScrollIcon=true;
      //     }else {
      //         this.isScrollIcon=false;
      //     }
      // },  
