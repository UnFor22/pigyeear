          ...
          <div class="contentBox" v-show="(isShow && index == i)">
            <a @click="tokefu(item.bank_name+'进度查询',item.prourl)" style="display: inline-block;width:100%;" v-if="item.prourl!=''">    
              <div class="online">
                ...
                <div>
                  <img src="../../assets/you@2x.png" alt="">
                </div>
              </div>
            </a>
            <div class="phone" @click="call(item.prophone)">
              <div>
                <p>电话查询</p>
                <p>{{item.procall}}</p>
              </div>
              <div>
                <a :href="'tel' + ':' + item.prophone"><img style="width: 60rpx; height: 40rpx" src="../../assets/dianhua@2x.png" alt=""></a>
              </div>
            </div>
          </div>
        </li>
      ...
      getProgress().then(data => {
        if(data.result.code == 10000){
          ...
          // Toast({message:'正在加载中。。。',duration: 500});
            ...
            /*console.log(this.pageList[ind].prophone);*/
        });
      }).catch(err => {
        console.log(err);
        // Toast({message:'正在加载中。。。',duration: 500});
